...
export async function validateGossipAttestationsSameAttData(
  ...
  const phase0ResultOrErrors = await Promise.all(
    attestationOrBytesArr.map((attestationOrBytes) => wrapError(phase0ValidationFn(chain, attestationOrBytes, subnet)))
  );
...
export async function validateAttestation(
  ...
  const phase0Result = await validateGossipAttestationNoSignatureCheck(chain, attestationOrBytes, subnet);
